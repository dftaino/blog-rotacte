...
export async function GET({ request }) {
  ...
  const [hoje, mes, porDia, top, leads, porMaterial, posts] = await Promise.all([
    q(`select coalesce(sum(acessos),0)::int t from pageview where dia = current_date`),
    q(`select coalesce(sum(acessos),0)::int t from pageview where dia >= current_date - 29`),
    q(`select dia, sum(acessos)::int acessos from pageview where dia >= current_date - 29 group by dia order by dia`),
    ...
    q(`select count(*)::int total,
              ...
              count(*) filter (where criado_em >= current_date)::int hoje from lead`),
